...
import { supabaseRpc, supabaseQuery, supabaseInsert, classifyPostgrestError } from "./supabase";
...
import type { ActionType, RiskTolerance } from "@mnemom/types";

// Canonical list of valid action types — must match the ActionType union in
// @mnemom/types. The drift test in tests/action-type-drift.test.ts asserts this.
...
// Compile-time exhaustiveness guard (MNE-414 advisory): VALID_ACTION_TYPES must
// cover exactly the @mnemom/types ActionType union. This is enforced here by `tsc`
// independent of ACTION_TYPE_PROFILES staying a total Record, so the drift
// protection cannot silently weaken if that map is ever made partial. A new
// ActionType member (or a stray value) fails the build at this line;
// tests/action-type-drift.test.ts is the runtime CI mirror.
type _ActionTypeDrift =
  | Exclude<ActionType, (typeof VALID_ACTION_TYPES)[number]>
  | Exclude<(typeof VALID_ACTION_TYPES)[number], ActionType>;
const _actionTypesExhaustive: [_ActionTypeDrift] extends [never] ? true : false = true;
...
/**
 * Resolve the billing account_id for an authenticated user, for tenant-scoping
 * the read (GET) endpoints. Mirrors the account resolution in
 * checkFeatureAndResolveAccount but without the feature-flag gate.
 *
 * Fail-open on resolution: if the billing lookup is unavailable, falls back to
 * the userId. On the read path this is the safe direction for a security
 * boundary — a row written while billing was UP (account_id = real billing id)
 * read back while billing is DOWN scopes to userId and yields a false 404
 * (deny), never a cross-tenant read. See Known Limitations in the plan.
 */
export async function resolveAccountId(env: RiskEnv, userId: string): Promise<string> {
  const { data, error } = await supabaseRpc(env, 'admin_get_billing_summary', {
    p_user_id: userId,
  });
  ...
  const summary = data as Record<string, unknown>;
  const account = summary.account as Record<string, unknown> | undefined;
  return (account?.account_id as string) ?? userId;
}
...
export async function handleAssessRisk(
  ...
  const { gateResponse, accountId } = await checkFeatureAndResolveAccount(
    env,
    userId,
    "risk_assessment",
  );
...
export async function handleAssessTeamRisk(
  ...
  if (agentIds.length < 2)
    return errorResponse("At least 2 agents required for team assessment", 400);
  ...
  const violationsMap = new Map<
    string,
    Array<{ severity: "low" | "medium" | "high" | "critical"; created_at: string }>
  >();
  ...
  const { data: pairwiseRows } = await supabaseQuery(env, "pairwise_coherence", {
    rawFilters: [`agent_a=in.(${idList})`, `agent_b=in.(${idList})`],
  });

  const pairwise: PairwiseCoherenceData[] = ((pairwiseRows ?? []) as Record<string, unknown>[]).map(
    (row) => ({
...
export async function handleGetAssessment(
  ...
  const { data, error } = await supabaseQuery(env, "risk_assessments", {
    filters: { assessment_id: assessmentId, account_id: accountId },
    single: true,
  });
...
export async function handleGetHistory(
  ...
  const { data, error, count } = await supabaseQuery(env, "risk_assessments", {
    filters: { agent_id: agentId, account_id: accountId },
...
export async function handleGetTeamHistory(
  ...
  const { data, error, count } = await supabaseQuery(env, "team_risk_assessments", {
    filters: { team_id: teamId, account_id: accountId },
    order: "created_at.desc",
    limit,
    offset,
  });
...
export async function handleGetProof(
  ...
  // risk_proofs rows carry no account_id of their own; ownership is verified
  // via the linked assessment. Select the owning table from proof_type and
  // confirm its account_id matches the caller. An unverifiable owner must
  // deny (404), never return the proof.
  const proof = data as Record<string, unknown>;
  const proofType = proof.proof_type as string | undefined;
  const assessmentId = proof.assessment_id as string | undefined;

  // Exhaustive set of known proof types (see ProofEnqueuedAttrs in metrics.ts).
  // Any other value — or a proof with no linked assessment — fails closed.
  let ownerTable: 'risk_assessments' | 'team_risk_assessments';
  if (proofType === 'individual_risk') {
    ownerTable = 'risk_assessments';
  } else if (proofType === 'team_coherence') {
    ownerTable = 'team_risk_assessments';
  } else {
    // Fail closed: unknown/unverifiable proof_type — deny rather than leak.
    return errorResponse('Proof not found', 404);
  }
  if (!assessmentId) return errorResponse('Proof not found', 404);

  const { data: ownerData, error: ownerError } = await supabaseQuery(env, ownerTable, {
    filters: { assessment_id: assessmentId, account_id: accountId },
    single: true,
  });

  if (ownerError) {
    const { status, message } = classifyPostgrestError(ownerError, 'Proof not found');
    return errorResponse(message, status);
  }
  if (!ownerData) return errorResponse('Proof not found', 404);
