...
export async function handleAssessTeamRisk(
  ...
  if (insertError) {
    // Persisted-store write failed but the assessment itself is valid. Signal
    // the partial success with a boolean flag (parity with handleAssessRisk);
    // log the raw error server-side only (never echo internal DB text to the
    // client — info-leak). Don't meter/prove a write that didn't persist.
    console.error('[handleAssessTeamRisk] Insert error:', insertError);
    return jsonResponse({ ...specTeamAssessment, _persistence_failed: true });
  }
